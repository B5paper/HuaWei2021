...
bool make_serv_full_load(int serv_id, int &num_mig, int max_num_mig, MigScheme &mig_scheme)
{
    ServStat &serv = serv_stats[serv_id];

    // 首先尝试能不能找一个虚拟机直接让其满载
    OneAssignScheme oas = mig_serv_select_vm_full_load(serv_id, 2, 1);
    if (oas.old_serv_id != -1)
    {
        mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, oas.server_node));
        serv_stats.mig_vm(oas.vm_id, oas.old_serv_id, oas.server_id, oas.server_node);
    ...
    // 若余量都小于 128 + thresh，那么试试迁出一个双节点虚拟机，看看还能不能使其满载
    if (serv.nodes[0].cores_ream <= 128 + core_node_full_load_thresh &&
        serv.nodes[0].mem_ream <= 128 + mem_node_full_load_thresh &&
        serv.nodes[1].cores_ream <= 128 + core_node_full_load_thresh &&
        serv.nodes[1].mem_ream <= 128 + mem_node_full_load_thresh)
    {
        ...
        for (auto &vm: vms_copy)
        {
            ...
            // 成功迁出去后，再尝试找虚拟机
            oas = mig_serv_select_vm_full_load(serv_id, 2, 1);
            if (oas.old_serv_id != -1)
            {
                mig_scheme.push_back(OneMigScheme(mig_out_vm_id, mig_out_new_serv_id, mig_out_new_serv_node));
                mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, 2));
                serv_stats.mig_vm(oas.vm_id, oas.old_serv_id, oas.server_id, oas.server_node);
                // succeed_to_mig_in = true;
                ++num_mig;
                return true;
...
void make_serv_full_load_2(int &num_mig, int num_max_mig, MigScheme &mig_scheme)
{
    static long hit_count;
    unordered_set<int> serv_searched;
    int vm_core, vm_mem, vm_type;
    int serv_core, serv_mem;
    int max_diff_vm_id;
    auto non_full_load_servs_copy(non_full_load_servs);
    for (auto serv_iter = non_full_load_servs_copy.rbegin(); serv_iter != non_full_load_servs_copy.rend(); ++serv_iter)
    {
        ServStat &serv = serv_stats[serv_iter->second];
        // 因为接下来服务器的状态会改变，所以可能之前是非空，现在变成了空的
        if (serv.is_empty())
            continue;

        // 首先看能不能找到个双节点虚拟机让它满载。单节点虚拟机是珍贵的资源，所以我们把它留到后面再用。
        ...
        // 若无法找到双节点虚拟机让它满载，那么要么这个服务器还远未装满，要么出现了不平衡的现象，可以是节点的不平衡，可以是核内的不平衡
        // 若两种资源都未满载，且内存数在 [serv_core * (mem_per_core - 1), serv_core * (mem_per_core + 1)) 区间内，这样的服务器我们称之为远未装满
        // 若两种资源有其中一种满载，另一种不满载，或者内存数不满足上述的要求，我们称之为核内的不平衡
        // 我们按照双节点服务器和单节点服务器实施两种不同的方案

        // 判断其是否是双节点服务器，双节点的服务器装的肯定有双节点虚拟机
        if (serv.nodes[0].cores_ream == serv.nodes[1].cores_ream && serv.nodes[0].mem_ream == serv.nodes[1].mem_ream)
        {
            // 如果这台服务器远未满载，我们跳过这样的服务器
            if (serv.nodes[0].cores_ream > core_node_full_load_thresh && serv.nodes[0].mem_ream > mem_node_full_load_thresh
                && serv.nodes[0].mem_ream >= serv.nodes[0].cores_ream * (mem_per_core - 1)
                && serv.nodes[1].mem_ream < serv.nodes[0].cores_ream * (mem_per_core + 1))
            {
                // 或许可以使用 balance fit，找到一个虚拟机使其满足 balance 需求，减少其余额
                continue;
            }

            // 迁出去一个与服务器型号核内比相差最大的虚拟机
            vector<pair<int, int>> cm_diffs;
            for (auto &vm_iter: serv.vms)
            {
                if (vm_iter.second.node != 2)
                    continue;

                vm_type = vm_iter.second.type;
                vm_core = vm_specs[vm_type].cores;
                vm_mem = vm_specs[vm_type].memcap;
                serv_core = serv_specs[serv.type].cores;
                serv_mem = serv_specs[serv.type].memcap;

                cm_diffs.emplace_back(abs((float) (vm_core - vm_mem) / (vm_core + vm_mem) 
                    - (float) (serv_core - serv_mem) / (serv_core + serv_mem)), vm_iter.first);
            }

            if (!cm_diffs.empty())
            {
                sort(cm_diffs.begin(), cm_diffs.end());
                max_diff_vm_id = cm_diffs.rbegin()->second;

                // 找一个服务器迁过去
                // 或许这里可以改为 balance fit
                OneAssignScheme oas = mig_vm_select_serv_first_fit(max_diff_vm_id, serv.id, serv_searched);
                if (oas.server_id != -1)
                {
                    mig_scheme.push_back(OneMigScheme(max_diff_vm_id, oas.server_id, oas.server_node));
                    serv_stats.mig_vm(max_diff_vm_id, serv.id, oas.server_id, oas.server_node);
                    ++num_mig;
                    if (num_mig >= num_max_mig)
                        return;
                    ...
                    // 然后再找一个双节点虚拟机迁进来
                    // 首先找能满足满载需求的
                    oas = mig_serv_select_vm_full_load(serv.id, 2, 2);
                    if (oas.server_id != -1)
                    {
                        mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, oas.server_node));
                        serv_stats.mig_vm(oas.vm_id, oas.server_id, oas.server_node);
                        ++num_mig;
                        if (num_mig >= num_max_mig)
                            return;
                        continue;
                    ...
                    // 若找不到满足满载需求的，那么尝试找个双节点的虚拟机减小不平衡度
                    // 这个函数还没有实现完全
                    oas = mig_serv_select_vm_best_fit(serv.nodes[0].cores_ream, serv.nodes[0].mem_ream, 2, serv.id, serv_searched);
                    if (oas.server_id != -1)
                    {
                        mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, oas.server_node));
                        serv_stats.mig_vm(oas.vm_id, oas.server_id, oas.server_node);
            ...
            // 若处理完后，此双节点服务器满足满载要求，或者远未满载，那么跳过此双节点服务服务器
            // 否则把它当作两个单节点服务器处理
            if (serv.nodes[0].cores_ream > core_node_full_load_thresh && serv.nodes[0].mem_ream > mem_node_full_load_thresh
                && serv.nodes[0].mem_ream >= serv.nodes[0].cores_ream * (mem_per_core - 1)
                && serv.nodes[1].mem_ream < serv.nodes[0].cores_ream * (mem_per_core + 1))
        ...
        // 对于非双节点服务器，或者处理完后仍不平衡的双节点服务器，我们把它当作两个单节点服务器处理
        for (int i = 0; i < 2; ++i)
        {
            // 若本节点满足满载要求，那么跳过
            ...
            // 若本节点两种资源都不满足满载需求，并且余量满足资源平衡条件，那么尝试迁入个虚拟机，目前先跳过
            if (serv.nodes[i].cores_ream > core_node_full_load_thresh
                && serv.nodes[i].mem_ream > mem_node_full_load_thresh
                && serv.nodes[i].mem_ream >= serv.nodes[i].cores_ream * (mem_per_core - 1)
                && serv.nodes[i].mem_ream < serv.nodes[i].cores_ream * (mem_per_core + 1))
            {
                // 首先尝试能否找到个单节点的虚拟机使其满足满载条件
                ...
                // 若找不到，那么找个虚拟机减少其余量（这里还没写完）
                // oas = mig_serv_select_vm_best_fit(serv.nodes[i].cores_ream, serv.nodes[i].mem_ream, i, serv.id, serv_searched);
                // if (oas.server_id != -1)
                // {
                //     mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, oas.server_node));
                //     serv_stats.mig_vm(oas.vm_id, oas.old_serv_id, oas.server_id, oas.server_node);
            ...
            // 剩下的情况基本就是资源不平衡的情况了，处理方式和资源不平衡的双节点服务器一致
            vector<pair<int, int>> cm_diffs;
            for (auto &vm: serv.vms)
            {
                if (vm.second.node == i)
                {
                    vm_type = vm.second.type;
                    vm_core = vm_specs[vm_type].cores;
                    vm_mem = vm_specs[vm_type].memcap;
                    serv_core = serv.nodes[i].cores_ream;
                    serv_mem = serv.nodes[i].mem_ream;

                    cm_diffs.emplace_back(abs((float) (vm_core - vm_mem) / (vm_core + vm_mem) - 
                        (float) (serv_core - serv_mem) / (serv_core + serv_mem)), vm.first);
                }
            }

            if (!cm_diffs.empty())
            {
                int max_diff_vm_id = cm_diffs.rbegin()->second;
                ...
                if (oas.server_id != -1)
                {
                    ...
                    // 然后再找一个双节点虚拟机迁进来
                    // 首先找能满足满载需求的
                    oas = mig_serv_select_vm_full_load(serv.id, i, 1);
                    if (oas.server_id != -1)
                    {
                        mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, oas.server_node));
                        serv_stats.mig_vm(oas.vm_id, oas.server_id, oas.server_node);
                    ...
                    // 若找不到满足满载需求的，那么尝试找个单节点的虚拟机减小不平衡度
                    // 这个函数还没有实现完全
                    oas = mig_serv_select_vm_best_fit(serv.nodes[i].cores_ream, serv.nodes[i].mem_ream, i, serv.id, serv_searched);
                    if (oas.server_id != -1)
                    {
...
void make_serv_full_load(int &num_mig, int num_max_mig, MigScheme &mig_scheme)
{
    ...
    for (auto non_iter = non_full_load_servs_copy.rbegin(); non_iter != non_full_load_servs_copy.rend(); ++non_iter)
    {
        ...
        OneAssignScheme oas = mig_serv_select_vm_full_load(serv.id, 2, 2);
        // OneAssignScheme oas = mig_serv_select_vm_best_fit(min(serv.nodes[0].cores_ream, serv.nodes[1].cores_ream),
        //     min(serv.nodes[0].mem_ream, serv.nodes[1].mem_ream), 2, serv.id, empty_set);
        if (oas.old_serv_id != -1)
        {
            ++hit_count;
            if (DEBUG)
            {
                if (hit_count % 100 == 0)
                {
                    cout << "two node full load hit count: " << hit_count << endl;
                }
            }
        ...
        {
            OneAssignScheme oas = mig_serv_select_vm_full_load(serv.id, i, 1);
            if (oas.old_serv_id != -1 && !serv_stats[oas.old_serv_id].is_full_load())
            {
                int vm_id = oas.vm_id;
                mig_scheme.push_back(OneMigScheme(oas.vm_id, oas.server_id, oas.server_node));
                serv_stats.mig_vm(oas.vm_id, oas.old_serv_id, oas.server_id, oas.server_node);
...
void mig_out(MigScheme &mig_scheme, int &num_mig, int num_max_mig)  // 往外迁效果不好
{
    unordered_set<int> serv_searched;
    ...
    {
        res_to_id.emplace_back(serv.second.nodes[0].cores_ream + serv.second.nodes[0].mem_ream + 
            serv.second.nodes[1].cores_ream + serv.second.nodes[1].mem_ream, serv.second.id);
    }
    sort(res_to_id.begin(), res_to_id.end());

    for (auto &res_id: res_to_id)
    {
        ServStat &serv = serv_stats[res_id.second];
        // serv_searched.insert(serv.id);
        auto vms_copy(serv.vms);
        // int num_vm_mig = min(2, int(vms_copy.size()));
        ...
        for (auto &vm: vms_copy)
        {
            ...
            // OneAssignScheme oas = mig_vm_select_serv_first_fit(vm.first, serv.id, serv_searched);
...
void migrate(MigScheme &mig_scheme, DayReqList &reqs, int idx_day)
{
    ...
    // make_serv_full_load_2(num_mig, max_num_mig * min(1.3 * add_total_ratio, 1.0), mig_scheme);
    // make_serv_full_load_2(num_mig, max_num_mig * min(0.2 * add_total_ratio, 1.0), mig_scheme);
