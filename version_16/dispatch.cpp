    ...
    ServStatList &serv_stats, ServSpecList &serv_specs, VMSpecList &vm_specs, DayReqList &reqs)
{
    ...
    while (true)
    {
        ...
        oas = add_vm_select_serv_bf_hash(vm_id, vm_type);
        ...
        if (oas.server_id == -1)
        {
            ...
            int num_add_reqs = 0;
            int num_del_reqs = 0;
            int num_total_reqs = 0;
            static double add_ratio;
            int num_cores = 0;
            int num_mems = 0;
            ...
            if (idx_day != current_day)
            {
                ...
                add_ratio = (double) (num_add_reqs - num_del_reqs) / (num_total_reqs);
            ...
            if (DEBUG)
                cout << "day " << idx_day << ", add ratio: " << add_ratio << ", core_mem_ratio: " << core_mem_ratio << endl;
            ...
            for (int i = 0; i < serv_specs.size(); ++i)
            {
                if (serv_specs[i].can_hold_vm(vm_type, vm_specs))
                {
                    ...
                    price = (2.5 + core_mem_ratio) * (serv_specs[i].cost + serv_specs[i].consume * (num_days - idx_day)) / serv_specs[i].cores
                        + 1.0 * (serv_specs[i].cost + serv_specs[i].consume * (num_days - idx_day)) / serv_specs[i].memcap;
                    if (price < min_price)
                    {
                        min_price = price;
                        min_idx = i;
