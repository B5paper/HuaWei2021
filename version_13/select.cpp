...
OneAssignScheme mig_vm_select_serv_bf_hash(int vm_id, int vm_type, int old_serv_id, unordered_set<int> &serv_searched, ServStatList &serv_stats, VMSpecList &vm_specs)
{
    ...
    if (vm_node == 1)
    {
        ...
        {
            auto iter = serv_core_mem_map.find(demand + shift);
            if (iter != serv_core_mem_map.end())
            {
    ...
    if (vm_node == 2)
    {
        ...
        while (shift < max_shift)
        {
            auto iter = dserv_core_mem_map.find(demand + shift);
            if (iter != dserv_core_mem_map.end())
            {
                for (int i = 0; i < iter->second.size(); ++i)
                {
