...
map<int, set<pair<int, pair<int, int>>>> serv_core_map_2;  // node core mem
...
map<int, set<pair<int, int>>> vm_core_map_2;  // core -> (vm_id, vm_type)
map<int, vector<int>> vm_core_map;
map<int, vector<int>> vm_mem_map;
map<int, vector<int>> dvm_core_mem_map;
map<int, set<pair<int, int>>> dvm_core_map_2;
...
void ServStat::del_vm(int vm_id, VMSpecList &vm_specs)
{
    ...
    {
        // auto iter = 
        serv_core_map_2[nodes[vm_node].cores_ream].erase(make_pair(nodes[vm_node].mem_ream, make_pair(id, vm_node)));
        if (serv_core_map_2[nodes[vm_node].cores_ream].empty())
            serv_core_map_2.erase(nodes[vm_node].cores_ream);
    }
    else
    {
        for (int i = 0; i < 2; ++i)
        {
            serv_core_map_2[nodes[i].cores_ream].erase(make_pair(nodes[i].mem_ream, make_pair(id, i)));
            if (serv_core_map_2[nodes[i].cores_ream].empty())
                serv_core_map_2.erase(nodes[i].cores_ream);
        }
    }
    ...
    if (vm_node < 2)
        serv_core_map_2[nodes[vm_node].cores_ream].emplace(nodes[vm_node].mem_ream, make_pair(id, vm_node));
    else
    {
        for (int i = 0; i < 2; ++i)
        {
            serv_core_map_2[nodes[i].cores_ream].emplace(nodes[i].mem_ream, make_pair(id, i));
        }
    }
    ...
    if (vm_node < 2)
        vm_core_map_2[vm_specs[vm_type].cores].erase(make_pair(vm_id, vm_type));
    else
        dvm_core_map_2[vm_specs[vm_type].cores].erase(make_pair(vm_id, vm_type));
...
void ServStat::add_vm(int vm_type, int vm_id, int serv_node, VMSpecList &vm_specs)
{
    ...
    if (serv_node < 2)
    {
        serv_core_map_2[nodes[serv_node].cores_ream].erase(make_pair(nodes[serv_node].mem_ream, make_pair(id, serv_node)));
        if (serv_core_map_2[nodes[serv_node].cores_ream].empty())
            serv_core_map_2.erase(nodes[serv_node].cores_ream);
    }
    else
    {
        for (int i = 0; i < 2; ++i)
    ...
    if (serv_node < 2)
        serv_core_map_2[nodes[serv_node].cores_ream].emplace(nodes[serv_node].mem_ream, make_pair(id, serv_node));
    else
    {
        for (int i = 0; i < 2; ++i)
        {
    ...
    if (serv_node < 2)
        vm_core_map_2[vm_specs[vm_type].cores].emplace(vm_id, vm_type);
    else
        dvm_core_map_2[vm_specs[vm_type].cores].emplace(vm_id, vm_type);
}
...
{
    if (serv_stats[n1.first].nodes[n1.second].cores_ream < serv_stats[n2.first].nodes[n2.second].cores_ream)
        return true;
    else
        return false;
}
...
template<typename T>
bool comp_node_mem::operator()(const T &n1, const T &n2) const
{
    if (serv_stats[n1.first].nodes[n1.second].mem_ream < serv_stats[n2.first].nodes[n2.second].mem_ream)
        return true;
    return false;
}
...
void ServStatList::add_serv(int id, int type)
{
    ...
    for (int i = 0; i < 2; ++i)
        serv_core_map_2[serv.nodes[i].cores_ream].emplace(serv.nodes[i].mem_ream, make_pair(id, i));
}
