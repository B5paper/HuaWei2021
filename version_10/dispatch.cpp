...
void process_add_op(int vm_id, int vm_type, PurchaseScheme &purchase_scheme, OneAssignScheme &assign_scheme, ServStatList &serv_stats, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    while (true)
    {
        ...
        if (oas.server_id == -1)
        {
            ...
            serv_stats.servs.insert(make_pair(serv_stats.servs.size(), ServStat(min_idx, serv_specs)));
...
void dispatch(ServSpecList &serv_specs, VMSpecList &vm_specs, DayReqList &reqs, ServStatList &serv_stats, DayDispList &disps)
{
    for (int idx_day = 0; idx_day < reqs.size(); ++idx_day)
    {
        ...
        // cout << "day " << idx_day << endl;
        // for (auto iter = serv_stats.servs.begin(); iter != serv_stats.servs.end(); ++iter)
        // {
        //     cout << "id: " << iter->first << ", node 0 core ream: " << iter->second.nodes[0].cores_ream
        //     << ", mem ream: " << iter->second.nodes[0].mem_ream << ", node 1 core ream: " 
        //     << iter->second.nodes[1].cores_ream << ", node 1 mem ream: " << iter->second.nodes[1].mem_ream << endl;
        // }
        // cout << endl;
        ...
        for (int i = 0; i < purchase_scheme.size(); ++i)
        {
            ...
            for (int j = 0; j < 2; ++j)
            {
                ream_to_serv_id[serv_stats[i].nodes[j].cores_ream + serv_stats[i].nodes[j].mem_ream + j] = make_pair(serv_id, j);
            }
        }
