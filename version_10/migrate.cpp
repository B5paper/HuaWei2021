...
OneAssignScheme select_best_fit_serv(int vm_type, int vm_id, int idx_start, vector<IdNum> &id_nums, ServStatList &serv_stats, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    ...
    for (int i = idx_start; i < id_nums.size(); ++i)
    {
        if (vm_node == 1)
        {
            for (int j = 0; j < 2; ++j)
            {
                res = calc_res(vm_type, serv_stats[id_nums[i].serv_id], j, serv_specs, vm_specs);
                if (res != -1)
                {
                    if (res < min_res)
                    {
        ...
        if (vm_node == 2)
        {
            res = calc_res(vm_type, serv_stats[id_nums[i].serv_id], 2, serv_specs, vm_specs);
            if (res != -1)
            {
                if (res < min_res)
                {
...
void migrate_2(MigScheme &mig_scheme, ServStatList &serv_stats, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    ...
    int max_num_mig = min(int(float(num_vms) * 5 / 1000), 40);
    ...
    for (int i = 0; i < id_nums.size(); ++i)
    {
        ...
        for (auto vm_iter = serv_stat.vms.begin(); vm_iter != serv_stat.vms.end();)
        {
            ...
            OneAssignScheme one_assign_scheme = mig_vm_select_serv_bf_hash(vm_iter->first, vm_iter->second.type, serv_stats, serv_specs, vm_specs);
            if (one_assign_scheme.server_id != -1)
            {
        ...
        for (auto vm_iter = serv_stat.vms.begin(); vm_iter != serv_stat.vms.end(); )
        {
            ...
            while (j < id_nums.size())
            {
                ...
                if (vm_iter->second.node == 0 || vm_iter->second.node == 1)
                {
                    for (int k = 0; k < 2; ++k)
                    {
                        res = calc_res(vm_iter->second.type, serv_stats[id_nums[j].serv_id], k, serv_specs, vm_specs);
                        if (res == -1)
                            ++num_cant_hold;
                ...
                {
                    res = calc_res(vm_iter->second.type, serv_stats[id_nums[j].serv_id], 2, serv_specs, vm_specs);
                    if (res == -1)
                        ++num_cant_hold;
