...
float calc_rel_res(int vm_type, ServStat &serv_stat, int serv_node, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    ...
    if (serv_node == 0 || serv_node == 1)
    {
        float cores_res = (serv_stat.nodes[serv_node].cores_ream - vm_core) / cores_cap;
        float mem_res = (serv_stat.nodes[serv_node].mem_ream - vm_mem) / mem_cap;
    ...
    if (serv_node == 2)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            cores_ream += serv_stat.nodes[i].cores_ream;
            mem_ream += serv_stat.nodes[i].mem_ream;
        }
...
float calc_bal_ratio(int vm_type, ServStat &serv_stat, int serv_node, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    ...
    if (serv_node == 0 || serv_node == 1)
    {
        int serv_core_ream = serv_stat.nodes[serv_node].cores_ream;
        int serv_mem_ream = serv_stat.nodes[serv_node].mem_ream;
    ...
    if (serv_node == 2)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            serv_core_ream = serv_stat.nodes[i].cores_ream;
            serv_mem_ream = serv_stat.nodes[i].mem_ream;
...
OneAssignScheme mig_vm_select_serv_dp(int vm_id, int vm_type, int idx_serv, vector<IdNum> &id_nums, ServStatList &serv_stats, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    int vm_core = vm_specs[vm_type].cores;
    int vm_mem  = vm_specs[vm_type].memcap;
    int vm_node = vm_specs[vm_type].nodes;
...
OneAssignScheme mig_vm_select_serv_bf_hash(int vm_id, int vm_type, ServStatList &serv_stats, ServSpecList &serv_specs, VMSpecList &vm_specs)
{
    int vm_core = vm_specs[vm_type].cores;
    int vm_mem = vm_specs[vm_type].memcap;
    int vm_node = vm_specs[vm_type].nodes;
    ...
    if (vm_node == 1)
    {
        ...
        int max_shift = ream_to_serv_id.size() / 2;
        while (shift < max_shift)
        {
            iter = ream_to_serv_id.find(demand + shift);
            if (iter != ream_to_serv_id.end())
            {
                if (serv_stats.servs[iter->second.first].vms.find(vm_id) == serv_stats.servs[iter->second.first].vms.end())
                {
                    if (serv_stats.servs[iter->second.first].can_hold_vm(vm_core, vm_mem, iter->second.second))
                        return OneAssignScheme(vm_id, iter->second.first, iter->second.second, serv_stats.get_serv_by_vm_id(vm_id).id);
                }
            }
