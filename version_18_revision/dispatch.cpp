    ...
    PurchaseScheme &purchase_scheme, OneAssignScheme &assign_scheme, DayReqList &reqs)
{
    ...
    while (true)
    {
        ...
        if (oas.server_id == -1)
        {
            ...
            if (num_serv_core_res_global + num_serv_mem_res_global != 0)
                    res_core_mem_ratio = (double) (num_serv_core_res_global - num_serv_mem_res_global) / (num_serv_core_res_global + num_serv_mem_res_global);
                else
                    res_core_mem_ratio = 0.0;

            weighted_result = (core_mem_ratio * (double) (num_cores + num_mems) / (num_serv_core_res_global + num_serv_mem_res_global + num_cores + num_mems)
                     - res_core_mem_ratio * (double) (num_serv_core_res_global + num_serv_mem_res_global) / (num_serv_core_res_global + num_serv_mem_res_global + num_cores + num_mems)); 
            ...
            for (int i = 0; i < serv_specs.size(); ++i)
            {
                ...
                {
                    // price = ((2.3 + weighted_result) / serv_specs[i].cores + 1.0 / serv_specs[i].memcap) * (serv_specs[i].cost + serv_specs[i].consume * (num_days - idx_day));
