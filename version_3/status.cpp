...
int VMStat::_get_idx_by_id(int vm_id)
{
    ...
    // cout << "can't find specified vm id" << endl;
    return -1;
}
...
void VMStat::add_vm(int type, int vm_id, int server_id, int node, ServerSpecList &server_specs, VMSpecList &vm_specs)
{
    ...
    // id_to_idx.insert(make_pair(vm_id, vm_ids.size()));
...
void VMStat::del_vm(int vm_id)
{
    ...
    // id_to_idx.erase(id_to_idx.find(vm_id));
}
...
void ServerStat::update_stat(VMStat &vm_stat, VMSpecList &vm_specs)
{
    ...
    for (int i = 0; i < vm_stat.types.size(); ++i)
    {
        ...
        server_idx = vm_stat.server_ids[i];
...
bool ServerStat::add_vm(int vm_type, int server_id, int node, ServerSpecList &server_specs, VMSpecList &vm_specs)
{
    ...
    // if (server_specs.cores[types[server_id]] - cores_used[server_id] < vm_specs.cores[vm_type])
    //     return false;
    // if (server_specs.memcap[types[server_id]] - mem_used[server_id] < vm_specs.memcap[vm_type])
    //     return false;
    switch (node)
    {
        case 0:
        case 1:
        if (server_specs.cores[types[server_id]] / 2 - nodes[node][server_id]->cores_used < vm_specs.cores[vm_type])
            return false;
        if (server_specs.memcap[types[server_id]] / 2 - nodes[node][server_id]->mem_used < vm_specs.memcap[vm_type])
            return false;
        break;
        ...
        for (int i = 0; i < 2; ++i)
        {
            if (server_specs.cores[types[server_id]] / 2 - nodes[i][server_id]->cores_used < vm_specs.cores[vm_type])
                return false;
            if (server_specs.memcap[types[server_id]] / 2 - nodes[i][server_id]->mem_used < vm_specs.memcap[vm_type])
                return false;
        }
        break;
    }
